...
public class CentralControl {
    ...
    private int checkEmpty() {   // 비어있는 배열의 index 넘버를 반환
        ...
        /*
            ...
            실패를 나타낼 때 -1을 쓰는 경우가 많습니다. 하지만 0과 너무 가까운 수이다 보니 -100이든
            아예 return 값으로 나올 수 없을만한 음수를 지정하는 경우도 있는데,
            나중에 addDevice()메서드에 if(checkEmpty() == - 2914782)로 쓰고 싶지 않으면
            -1로 쓰는게 가장 보편적입니다.
         */
    }

    public void powerOn() {
        // 배열 내부에 있는 element들은(Power의 서브 클랫스들의 인스턴스) Power를 implment했기 때문에
     ...
     public void powerOff() {
         ...
         for(int i = 0 ; i < deviceArray.length ; i++) {
             if (deviceArray[i] == null) {
                 System.out.println("장치가 없어 전원을 끄지 못했습니다.");
                 continue;
             }
             deviceArray[i].off();
         }
