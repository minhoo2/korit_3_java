...
/*
    ...
    1. Setter : 속성값을 대입(변경)하기 위한 method를 통칭합니다.
        ...
        b. 내부에 로직을 작성하여 필드의 논리적인 개념에 맞지 않는
                데이터를 걸러 낼 수 있습니다.(배터리타임이 음수면 아예 메서드를 정지시킬 수 있음.
    ...
    2. Getter : 속성값을 조회하기 위한 method를 통칭
        ...
        ex) System.out.println(watch.getButton()+ "!"; // true!
        setter와 마찬가지로 alt + ins를 통해서 작성이 가능합니다.

    지사 사항
        1. 기본생성자 / 이름을 필수로 갖는 매개변수 생성자를 정의할 것
        2. 모든  필드에 대해 Setter를 정의할 것
        3. setBatteryTime()의 구현부에 batteryTime이 10미만이라면 method 정지시킬 것
        4. watch1에 setter를 활용하여,
            betton true
            celular true
            battrtyTime - 10 - > 실패하고 -> 18
            title 애플워치
            로 객체를 생성하시오.

        5. showInfo()를 정의하여
            이 시계는 애플워치입니다. 배터리 타임이 18이지요
            감사합니다.
            를 출력시킬 것
 */
class SmartWatch {
    ...
    public void setBatteryTime(int batteryTime) {
        // 10미만은 대입될 수 없도록 조건문 작성
        if (batteryTime < 10) {
            System.out.println("불가능한 배터리 타임 입력입니다.");
            return;
        }
